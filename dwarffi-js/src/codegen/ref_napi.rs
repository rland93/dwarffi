...
fn generate_struct(
    ...
    if is_opaque {
        ...
        output.push_str(&format!(
            "const {} = ref.types.void // treat as opaque handle\n\n",
            name
        ));
        return Ok(output);
    }
    ...
    for field in fields {
        let field_type = type_to_js_expr(type_registry, field.type_id, TypeContext::StructField)?;
...
fn type_to_js_expr(
    ...
    if matches!(context, TypeContext::FunctionParam) {
        ...
        if type_.pointer_depth == 1
            && type_.is_const
            && let BaseTypeKind::Primitive { name, .. } = &type_.kind
            && name == "char"
        {
...
fn generate_function_bindings(
    ...
    for func in functions {
        ...
        let param_types: Result<Vec<String>> = func
            .parameters
            .iter()
            .map(|p| type_to_js_expr(type_registry, p.type_id, TypeContext::FunctionParam))
            .collect();
